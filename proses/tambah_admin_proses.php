...
if ($foto != "") {
    ...
    if (in_array($ekstensi, $ekstensi_boleh) === true) {
        ...
        $q = "insert into users (nama, kelas, username, password, jenis_kelamin, foto, id_level, level)
              values ('$nama_admin', '$kelas', '$username', '$password', '$jenis_kelamin', '$nama_foto_baru', '$id_level', '$level')";
        $query = $conn->query($q);
        ...
        if (!$query) {
            ...
            $_SESSION['pesan'] = '<div class="alert alert-success" role="alert">Data admin berhasil ditambahkan</div>';
        }
    } else {
    ...
    //jika belum melakukan upload foto admin
    $q = "insert into users (nama, kelas, username, password, jenis_kelamin, foto, id_level, level)
            values ('$nama_admin', '$kelas', '$username', '$password', '$jenis_kelamin', '', '$id_level', '$level')";
    $query = $conn->query($q);
